...
const char *SkyMetaEngine::getEngineId() const {
    return "sky";
}
...
Common::KeymapArray SkyMetaEngine::initKeymaps(const char *target) const {
    ...
    // Note - if changing this, be sure to change it in sky.cpp for consistency.
    //const char *shortcutsKeymapId = "sky-shortcuts";
	Keymap *shortcutsKeymap = new Keymap(Keymap::kKeymapTypeGame, shortcutsKeymapId, "Beneath a Steel Sky - Shortcuts");
...
REGISTER_PLUGIN_STATIC(SKY, PLUGIN_TYPE_METAENGINE, SkyMetaEngine);

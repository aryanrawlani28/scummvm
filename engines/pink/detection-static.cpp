...
#include "pink/saveload.h" // Some functions necessary for listing the saves.
...
PinkMetaEngine::PinkMetaEngine() : AdvancedMetaEngine(Pink::gameDescriptions, sizeof(ADGameDescription), pinkGames) {
    _gameIds = pinkGames;
    _maxScanDepth = 2;
    _directoryGlobs = directoryGlobs;
}

const char *PinkMetaEngine::getEngineId() const {
    return "pink";
}

const char *PinkMetaEngine::getName() const {
    return "Pink Panther";
}

const char *PinkMetaEngine::getOriginalCopyright() const {
...
REGISTER_PLUGIN_STATIC(PINK, PLUGIN_TYPE_METAENGINE, PinkMetaEngine);
